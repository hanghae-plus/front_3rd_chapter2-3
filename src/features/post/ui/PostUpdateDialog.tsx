    ...
    <CustomDialog open={showPostUpdateDialog} onOpenChange={setShowPostUpdateDialog} title={"게시물 수정"}>
      <>
        <Input
          ...
          onChange={(e: ChangeEvent<HTMLInputElement>) => setUpdatingPost((prevPost) => ({ ...prevPost, title: e.target.value }))}
        />
        <Textarea
          rows={15}
          placeholder="내용"
          value={updatingPost.body}
          onChange={(e: ChangeEvent<HTMLInputElement>) => setUpdatingPost((prevPost) => ({ ...prevPost, body: e.target.value }))}
        />
