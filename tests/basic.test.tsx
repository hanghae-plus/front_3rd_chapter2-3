...
describe("PostsManager", () => {
  ...
  it("새 게시물 추가를 허용합니다", async () => {
    ...
    await waitFor(() => {
      TEST_POSTS.posts.forEach(post => {
        expect(screen.getByText(text => text.includes(post.title))).toBeInTheDocument();
      });
    });
    ...
    await waitFor(() => expect(screen.getByText(/새 게시물 추가/i)).toBeInTheDocument());

    const titleInput = await screen.getByPlaceholderText(/제목/i);
    const bodyInput = await screen.getByPlaceholderText(/내용/i);
